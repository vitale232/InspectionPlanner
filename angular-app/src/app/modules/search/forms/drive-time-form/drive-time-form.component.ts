...
export class DriveTimeFormComponent implements OnInit, OnDestroy {
  ...
  onSearch() {
    ...
    this.driveTimeQueriesService.getDriveTime(driveTimeQueryParams).subscribe(
      data => {
        if ((data.body as IDriveTimeQueryFeature).id) {
          console.log('existing drive time', data);
          this.onExistingDriveTimeQuery((data.body as IDriveTimeQueryFeature), driveTimeQueryParams.drive_time_hours);
        } else if ((data.body as ISubmittedDriveTimeQuery).msg === 'The request has been added to the queue') {
